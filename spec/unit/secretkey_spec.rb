  ...
  describe '.generate_token' do
    ...
    context 'when timestamp is invalid' do
      ...
      it 'raises an invalid timestamp error' do
        expect(lambda { described_class.generate_token(key, secret, time_stamp) }).to raise_error(ArgumentError, 'Time Stamp is not a valid integer.')
      end
    end
  end
